...
const Menu: React.FC<MenuProps> = ({ open, setOpen }) => {
  ...
  return (
    <>
      <div
        className={clsx(
          "justify-between items-center w-full h-screen md:h-full md:flex md:order-1 duration-200",
          !open ? "hidden" : ""
        )}
        id="navbar-sticky"
      >
        <div className="flex flex-col p-4 mt-4 bg-gray-50 rounded-lg border border-gray-100 md:p-2 md:flex-row md:space-x-8 md:mt-0 md:text-md md:font-medium md:border-0 md:bg-white">
        ...
        <div className="flex flex-col p-4 gap-5 md:p-2 md:flex-row md:mt-0 md:text-md md:font-medium md:border-0 md:items-center">
          <div>
            <NavLink
              to="/register"
